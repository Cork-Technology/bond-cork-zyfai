...
<!-- Sources of truth: cork/docs/examples/ — ceremony.md (flow + account-type rule),
     ...
     cork/src/CorkLimitOrderAdapter.sol for hook semantics. The examples are fork-proven against
...
Cork API is the book. **Never re-derive byte plumbing** — import from `cork/docs/examples/lib.mjs`
...
`KNOWN_ERRORS_ABI` + `failWith` for decoded reverts). Run scripts from `cork/docs/examples/`
...
2. **Bid** — the asker posts an opening BUY bid: `makerAsset = CA` (premium offered),
   ...
   about it. Script for steps 1+2 in one go: `cork/docs/examples/market-rfq.mjs`.
...
5. **Fill** — whoever moves first ends the negotiation: the asker fills the ask
   (`cork/docs/examples/jit-fill.mjs`), or the underwriter lifts the bid
   (`cork/docs/examples/lift-bid.mjs`).

The whole loop — both discovery modes, both account types — is one runnable script:
**`cork/docs/examples/ceremony-e2e.mjs`**; prose walkthrough: `cork/docs/examples/ceremony.md`.
...
Exact bytes: `cork/docs/examples/jit-order.mjs` (maker) / `jit-fill.mjs` (taker) /
...
`execTransaction` — see `cork/docs/examples/contract-maker.md`. Scripts:
`cork/docs/examples/contract-order.mjs` (maker; `VARIANT=PLAIN|JIT`) / `contract-fill.mjs` (taker).
Fork proof: `cork/test/fork/ContractMakerFill.fork.t.sol`.
...
construction: `orderPayload()` in `cork/docs/examples/ceremony-e2e.mjs`.
...
`cork/docs/examples/plain-order.mjs` (`SIDE=SELL|BUY`) / `plain-fill.mjs`; walkthrough
`cork/docs/examples/plain-order.md`.
